...
- [x] **Task 12: 手动验收清单 + 打磨 + 音效动效**
  - Acceptance: `docs/manual-test-checklist.md` 覆盖全流程；文案/间距/按钮尺寸按孩子视角打磨；金币数值按试玩反馈微调；按钮点击/文字提示带音效（Web Audio 合成 tap/correct/wrong/coin/gem/gacha/win）与动效（按压缩放/fadeUp/pop/bounce/shake）；音频首次交互解锁（解决 iOS/Android 自动播放拦截）
  - Verify: 清单全过；本地 CDP 回归无 undefined、音效路径可执行
